...
class StringSolver[Identifier] {
  ...
  @tailrec final def evaluate(s: Assignment, acc: StringBuffer = new StringBuffer(""))(sf: StringForm): String = sf match {
  ...
  @tailrec final def reduceStringForm(s: Assignment, acc: ListBuffer[StringFormToken] = ListBuffer())(sf: StringForm): StringForm = sf match {
    case Nil => acc.toList
  ...
  trait ProblemSimplicationPhase { self =>
    ...
    def loopUntilConvergence = new ProblemSimplicationPhase {
      def run(p: Problem, s: Assignment): Option[(Problem, Assignment)] = {
        fixpoint[Option[(Problem, Assignment)]]{
          case None => None
          case Some((problem, assignment)) => self.run(problem, assignment)
        }(Option((p, s)))
      }
    }
  }
  ...
  @tailrec final def statsStringForm(e: StringForm, acc: Map[Identifier, Int] = Map()): Map[Identifier, Int] = e match {
    case Nil => acc
    case Right(id)::q => statsStringForm(q, acc + (id -> (acc.getOrElse(id, 0) + 1)))
    case (_::q) => statsStringForm(q, acc)
  }
  
  @tailrec final def stats(p: Problem, acc: Map[Identifier, Int] = Map()): Map[Identifier, Int] = p match {
    case Nil => acc
    case (sf, rhs)::q => 
